...
namespace QIQO.Orders.Data
{
                                     ...
                                     IPersonTypeRepository
    {
        ...
        public PersonTypeRepository(IOrderDbContext dbc, IPersonTypeMap map, ILogger<PersonTypeData> log) : base(log, map)
        {
            entityContext = dbc;
        }
        ...
        public override void Delete(PersonTypeData entity)
        {
            ...
            using (entityContext) entityContext.ExecuteProcedureNonQuery("usp_person_type_del", Mapper.MapParamsForDelete(entity));
        }

        public override void DeleteByCode(string entityCode)
        {
            Log.LogInformation("Accessing PersonTypeRepo DeleteByCode function");
            var pcol = new List<SqlParameter>() { Mapper.BuildParam("@person_type_code", entityCode) };
        ...
        {
            Log.LogInformation("Accessing PersonTypeRepo Delete function");
            using (entityContext) entityContext.ExecuteProcedureNonQuery("usp_person_type_del", Mapper.MapParamsForDelete(entityKey));
        }

        private void Upsert(PersonTypeData entity)
        {
            using (entityContext) entityContext.ExecuteProcedureNonQuery("usp_person_type_ups", Mapper.MapParamsForUpsert(entity));
        }
    }
}
